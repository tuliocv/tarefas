...
class AIInsights:
    ...
    def sentimento_historico(self):
        """Analisa o sentimento dos textos"""
    ...
    def recomendacoes(self):
        """Gera insights com base nos dados"""
        if self.df.empty:
            return st.info("Sem dados suficientes para recomendações.")
        ...
        if (self.df["status"] == "Concluída").sum() == 0:
            st.write("- Nenhuma tarefa concluída ainda — defina pequenas metas diárias para começar.")
